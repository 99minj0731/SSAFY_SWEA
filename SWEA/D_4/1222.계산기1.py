...
def postfix(numbers):  #후위 계산식을 만드는 함수
...
def postfix_2(nums):   #후위 계산식을 계산하는 함수
    ...
    for token in nums:
        if token == '+':     #'+'가 나온다면
            ...
            # top -= 1   #탑 하나 내려
            stack2[top] = int(right_num) + int(left_num) #그리구 거기에 두 값의 합을 넣어준다
        else:   # 스트링이지만 숫자인척하는 애들이 나온다면
            top += 1  #탑을 하나 올리고
            stack2[top] = token

    return int(stack2[0]) + int(stack2[1])   #마지막 숫자가 더하기가 안되길래 그냥 뒤에 더해줬다
...
for test_case in range(1, 11):
    ...
    cal_change = postfix(arr) #['9', '8', '+', '5', '+', '9' .....'+', '2', '+', '6', '+', '6']
